...
if ( ! function_exists( 'bt_get_tour_single_data' ) ) {
    function bt_get_tour_single_data( $post_id = 0 ) { 
       if ( function_exists( 'boldthemes_get_option' ) ) {   
            ...
            if ( $post_content != '' ) {
				$tour_title = BoldThemesFrameworkTemplate::$title;
				$tour_excerpt = BoldThemesFrameworkTemplate::$excerpt;
                $content_html = apply_filters( 'the_content', $post_content ) ; 
				BoldThemesFrameworkTemplate::$title = $tour_title;
				BoldThemesFrameworkTemplate::$excerpt = $tour_excerpt;       
                //$content_html = apply_filters( 'the_content', $post_content ) ;            
...
if ( ! function_exists( 'bt_get_tour_tags_by_slugs' ) ) {
    function bt_get_tour_tags_by_slugs( $tag = '', $delimeter = ';' ) {
        $search_tags = array();
        if ( $tag != '' ){
            $_tags = explode( $delimeter, $tag );
            if ( is_array($_tags) && !empty($_tags) ){                        
                foreach ( $_tags as $_tag ){ 
                    $_tag_term = get_term_by('slug', trim($_tag), 'tour_tag');
                    if ( isset($_tag_term) && !empty($_tag_term) ){
                        array_push( $search_tags, $_tag_term->term_id );
                    }
                }                        
            }
        }        
