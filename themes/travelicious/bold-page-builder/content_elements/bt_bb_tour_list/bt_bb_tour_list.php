...
class bt_bb_tour_list extends BT_BB_Element {
		...
		$tag       		= sanitize_text_field( $tag );//slugs
		...
		BoldThemesFrameworkTemplate::$search_tags		    = bt_get_tour_tags_by_slugs( $tag );
		...
		if ( $sorting != '' && intval($sorting) > -1 ){
			...
			BoldThemesFrameworkTemplate::$tours  = boldthemes_get_query_items(
					array(
							...
							'search_tags'		 => BoldThemesFrameworkTemplate::$search_tags,//ids
		...
		if ( $view_type == 'btListDesignTiles' ){                    
			...
			$output .= '<div class="bt_bb_grid_hide bt_bb_masonry_tour_list_content" data-number-posts="' . esc_attr( $number ) . '" data-format-posts="' . esc_attr( $format ) . '" data-category-posts="' . esc_attr( $category ) . '" data-tag-posts="' . esc_attr( $tag ) . '"><div class="bt_bb_grid_sizer bt_bb_grid_sizer masonry-brick"></div>';
				ob_start();
        ...
        function map_shortcode() {
            ...
            bt_bb_map( $this->shortcode, array( 'name' => esc_html__( 'Tour List', 'travelicious' ), 'description' => esc_html__( 'List of Tours Presentation', 'travelicious' ), 'container' => 'vertical', 'icon' => $this->prefix_backend . 'icon' . '_' . $this->shortcode,
                    'params' => array(
							...
							array( 'param_name' => 'tag', 'type' => 'textfield', 'heading' => esc_html__( 'Tag', 'travelicious' ), 'description' => esc_html__( 'Enter tag slug or tags slugs separated by ; or leave empty to show all', 'travelicious' ), 'preview' => true ),
