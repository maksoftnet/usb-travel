...
if ( ! function_exists( 'boldthemes_search_form' ) ) {
	function boldthemes_search_form( $form ) {
		...
		$form .= boldthemes_get_icon_html( array( 'icon' => 'fa_f002', 'url' => '#', 'url_title' => esc_attr__( 'Search', 'travelicious' ) ) );
		$form .= '
		<div class="btSearchInner gutter" role="search">
			<div class="btSearchInnerContent port">
				<form action="' . esc_attr( home_url( '/' ) ) . '" method="get"><input type="text" name="s" placeholder="' . esc_attr__( 'Looking for...', 'travelicious' ) . '" class="untouched">
...
if ( ! function_exists( 'boldthemes_wp_audio_shortcode_library' ) ) {
	function boldthemes_wp_audio_shortcode_library() {
		wp_enqueue_style( 'wp-mediaelement' );
		wp_enqueue_script( 'boldthemes-audio-shortcode', get_parent_theme_file_uri( 'framework/js/audio_shortcode.js' ), array(), '', true );
		return 'boldthemes_mejs';
	}
}
	...
	function boldthemes_allowed_html( $tags, $context ) {
		if ( $context == 'tags' ) {
			...
			$allowed_attributes = array(
				...
				'style' => true,
				'data-ico-fa' => true,
				'data-ico-fontawesome5brands' => true,
...
if ( ! function_exists( 'boldthemes_override_unload_textdomain' ) ) {
	function boldthemes_override_unload_textdomain( $override, $domain ) {
		if ( $domain === 'travelicious' || $domain === 'bt_plugin' ) {
